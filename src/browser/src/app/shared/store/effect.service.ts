...
export class EffectService {
  constructor(
    ...
    // * update title
    document.title = this.store.getCurrentWorkspace?.title ? `POSTDOG - ${this.store.getCurrentWorkspace?.title}` : 'POSTDOG';
  ...
  async init() {
    ...
    //User first use POSTDOG,select localwork space
    if (isUserFirstUse) {
    ...
    this.updateProjects(this.store.getCurrentWorkspaceUuid).then(async () => {
      // Use first user POSTDOG,auto into Default project
      if (isUserFirstUse) {
        //* Prevent 404
        waitNextTick().then(() => {
