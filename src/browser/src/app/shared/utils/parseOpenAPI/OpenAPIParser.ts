...
export class OpenAPIParser {
  ...
  schema2POSTDOGiEditBody(schema?: OpenAPIV3.ReferenceObject | OpenAPIV3.SchemaObject): BodyParam[] {
    if (!schema) return [];
    ...
    if (this.is$ref(schema)) {
      ...
      return schemaObject
        ? this.schema2POSTDOGiEditBody(schemaObject).concat(this.transformProperties(innerSchema?.properties, innerSchema?.required))
        : [];
  ...
  generateBody(body?: OpenAPIV3.ReferenceObject | OpenAPIV3.RequestBodyObject | OpenAPIV3.ResponseObject): [] | BodyParam[] {
    ...
    if (this.is$ref(body)) {
      const [ref] = this.get$Ref(body);
      const schemaObject = this.getSchemaBy$ref(ref);
      return schemaObject ? this.schema2POSTDOGiEditBody(schemaObject) : [];
    } else if (body?.content) {
      const media = Object.values(body.content).at(0);
      return media?.schema ? this.schema2POSTDOGiEditBody(media?.schema) : [];
    }
    return this.schema2POSTDOGiEditBody(body);
  }

  generateResponseBody(responses: OpenAPIV3.ResponsesObject) {
    const resObj = this.getResponseObject(responses);
    if (resObj?.content) {
      return this.schema2POSTDOGiEditBody(Object.values(resObj.content).at(0)?.schema);
    } else {
      return [];
    }
  }
