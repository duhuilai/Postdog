...
export class ExtensionDetailComponent implements OnInit {
  ...
  jumpToClient() {
    this.webService.jumpToClient($localize`POSTDOG Client is required to use this extension.`);
  }
  async handleInstall() {
  ...
  async getDetail() {
    ...
    this.isAvailableVersion = compareVersions(pkgInfo.version, this.extensionDetail.engines?.POSTDOG) >= 0 ? true : false;
