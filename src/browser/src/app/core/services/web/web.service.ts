  ...
  resourceInfo = [
    {
      ...
      keyword: 'Setup',
      suffix: 'exe',
      link: 'https://data.POSTDOG.com/download/latest/POSTDOG-Setup-latest.exe'
    },
    {
      id: 'mac-intel',
      ...
      icon: 'mac',
      suffix: 'dmg',
      link: 'https://data.POSTDOG.com/download/latest/POSTDOG-latest.dmg'
    },
    {
      id: 'mac-m1',
      ...
      icon: 'mac',
      suffix: 'arm64.dmg',
      link: 'https://data.POSTDOG.com/download/latest/POSTDOG-latest-arm64.dmg'
    }
  ];
  githubBugUrl: string;
  constructor(
    ...
    switch (platform) {
      case 'gitee': {
        this.resourceInfo.forEach(item => {
          switch (item.id) {
            case 'win': {
              item.link = `https://gitee.com/eolink_admin/POSTDOG/releases/download/v${packageJson.version}/POSTDOG-Setup-${packageJson.version}.exe`;
              break;
            }
            case 'mac-intel': {
              item.link = `https://gitee.com/eolink_admin/POSTDOG/releases/download/v${packageJson.version}/POSTDOG-${packageJson.version}.dmg`;
              break;
            }
            case 'mac-m1': {
              item.link = `https://gitee.com/eolink_admin/POSTDOG/releases/download/v${packageJson.version}/POSTDOG-${packageJson.version}-arm64.dmg`;
              break;
            }
          }
        });
      ...
      case 'github': {
        fetch('https://api.github.com/repos/Postdoglab/postdog/releases')
          ...
          .catch(e => {
            pcConsole.log('get POSTDOG client download link error');
          });
        break;
      }
    }
